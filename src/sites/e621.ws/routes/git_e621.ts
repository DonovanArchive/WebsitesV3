...
async function tagPush(data: PushEvent) {
	const tmp = `${tmpdir()}/${await mkdtemp("e621.")}`;
	...
	await git
		.init()
		.addRemote("origin", "https://github.com/DonovanDMC/E621")
...
exports.load = function load(app) {\n\
	...
	await writeFile(`${tmp}/typedoc.json`, JSON.stringify({
		...
		customTitleLink: "https://npm.e621.ws/e621"
	}));
