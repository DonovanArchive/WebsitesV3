import type { EmbedField, EmbedOptions } from "eris";

export default class EmbedBuilder {
	private json: EmbedOptions & { timestamp?: string; };
	constructor(json: EmbedOptions & { timestamp?: string; } = {}) {
		this.json = json ?? {};
	}
	...
	setTimestamp(time: string | Date | "now") {
		...
		this.json.timestamp = time instanceof Date ? time.toISOString() : time;
		return this;
	}
	...
	toJSON(array: true): Array<EmbedOptions & { timestamp?: string; }>;
	toJSON(array?: false): EmbedOptions & { timestamp?: string; };
