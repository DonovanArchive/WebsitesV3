...
app
	...
	.get("/:id", async(req, res) => {
		...
		if (!gifExists && !pngExists) return res.status(404).json({ success: false, error: "Not Found" });
	...
	.put("/:id/:type", async(req, res) => {
		...
		if (existing) return res.status(200).json({ success: true, url: url(md5, type) });
		...
		return res.status(202).json({ success: true, status: "processing", checkURL: check.url, time: check.time, startedAt: check.startedAt });
	})
